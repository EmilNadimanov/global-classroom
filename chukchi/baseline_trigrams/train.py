...
for token1 in trigram_counts:
	...
	for token2 in trigram_counts[token1]:
		...
		token_total = sum(trigram_counts[token1][token2].values())
		for token3 in trigram_counts[token1][token2]:
			if token3 not in trigrams[token1][token2]:
				trigrams[token1][token2][token3] = 0
				n_trigrams += 1
			trigrams[token1][token2][token3] = trigram_counts[token1][token2][token3]/token_total

# Write out model
mf = open(model_file, 'wb')
pickle.dump((unigrams, bigrams, trigrams), mf)
print('Baseline with trigrams')
print('Written %d unigrams, %d bigrams and %d trigrams to %s.' % (len(unigrams.keys()), n_bigrams, n_trigrams, model_file))
